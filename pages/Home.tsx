...
const Home: React.FC = () => {
  ...
  if (loading || !coupleData) return <div className="h-screen flex items-center justify-center text-[#D9B26D]">載入中...</div>;

  return (
    <div className="min-h-full flex flex-col pt-0 px-0 pb-24 relative bg-[#F7F3ED]">
      <input type="file" ref={fileInputRef} onChange={handleAvatarUpload} className="hidden" accept="image/*" />
      <input type="file" ref={coverInputRef} onChange={handleCoverUpload} className="hidden" accept="image/*" />

      {/* Top Cover Section */}
      <div className="relative w-full h-[280px] rounded-b-[40px] overflow-hidden shadow-lg border-b-4 border-white">
         <img 
         ...
         {/* Status Picker Menu */}
         {showStatusMenu && (
           <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 bg-white rounded-2xl shadow-xl p-3 grid grid-cols-4 gap-2 z-20 animate-slide-up">
              {statusOptions.map(s => (
                <button key={s.label} onClick={() => updateStatus(s.label + ' ' + s.icon)} className="flex flex-col items-center gap-1 p-2 hover:bg-[#F7F3ED] rounded-lg">
                  <span className="text-xl">{s.icon}</span>
                  <span className="text-[10px] text-[#3A3A3A]">{s.label}</span>
                </button>
              ))}
           </div>
         )}
      ...
      <div className="px-6 -mt-6 relative z-10">
         ...
         <div className="bg-white rounded-[32px] shadow-xl shadow-[#D9B26D]/10 overflow-hidden relative aspect-[4/5] w-full max-h-[400px] border border-white mx-auto">
          {todayEvent ? (
            <div 
              onClick={() => navigate(`/event-edit/${todayEvent.id}`)} // Fixed to go to edit
