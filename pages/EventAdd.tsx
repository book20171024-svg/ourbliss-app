...
const EventAdd: React.FC = () => {
  ...
  return (
    <div className="h-full bg-[#F7F3ED] flex flex-col animate-fade-in relative">
      {/* Header - Fixed */}
      <div className="px-6 py-4 flex justify-between items-center bg-[#F7F3ED] z-10 flex-shrink-0">
        <h2 className="text-xl font-serif font-bold text-[#3A3A3A]">新增活動</h2>
        <button onClick={() => navigate(-1)} className="p-2 bg-white rounded-full text-[#C1C1C1] active:scale-95 transition-transform"><X size={20} /></button>
      </div>
            ...
            <div className="flex gap-2">
              {[
                ...
                <button
                  ...
                  className={`flex-1 py-2 rounded-xl flex items-center justify-center gap-2 border transition-colors ${
      ...
      {/* Footer Button - Fixed */}
      <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-[#F7F3ED] via-[#F7F3ED]/90 to-transparent z-10 safe-area-bottom">
        <button 
          type="submit" 
          form="event-form"
          disabled={isSubmitting} 
          className="w-full bg-[#D9B26D] text-white py-4 rounded-full font-bold shadow-lg shadow-[#D9B26D]/20 active:scale-95 transition-transform"
        >
          {isSubmitting ? '建立中...' : '完成'}
        </button>
      </div>
